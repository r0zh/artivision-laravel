    ...
    <div class="p-4 columns-1 sm:columns-2 md:columns-3 lg:columns-4 xl:columns-5">
        @foreach ($images as $image)
            <div class="relative  py-2 px-2 rounded-md hover:bg-[#6e42c185] transition-all">
                <img src="@if ($image->public == 0) {{ $image->path }} @else {{ url($image->path) }} @endif"
                ...
                @if ($selectedImage == $image->id)
                    <div class="flex fixed top-0 left-0 z-10 justify-center items-center w-full h-full bg-black bg-opacity-50"
                        ...
                        <div class="grid relative grid-cols-2 gap-3 p-4 bg-white rounded-xl shadow-lg"
                            ...
                            <div>
                                <img src="@if ($image->public == 0) {{ $image->path }} @else {{ url($image->path) }} @endif"
                                    alt="{{ $image->positivePrompt }}" class="w-full h-auto">
                            </div>
                            ...
                            <svg class="absolute top-0 right-0 p-2 w-12 cursor-pointer" fill="#000000"
                                viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" wire:click="deselectImage">
                                <path
